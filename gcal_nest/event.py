...
class Event(object):
    ...
    def __str__(self):
        return "{0}: {1}".format(
            arrow.get(self.scheduled_date).format("YYYY-MM-DD HH:mm"), self.name
        )
    ...
    def from_db(self, db_dict):
        ...
        self.scheduled_date = arrow.get(db_dict["scheduled_date"]).to(
            db_dict["timezone"]
        )
    ...
    def from_gcal_dict(self, event, timezone):
        ...
        if "date" in event["start"]:
            self.scheduled_date = arrow.get(
                event["start"]["date"] + " " + default_start_time + " " + timezone,
                "YYYY-MM-DD H:mm ZZZ",
            )
        else:
